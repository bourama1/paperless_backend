...
export const reviseFile = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const latestRevision = await db("revisions").where("document_id", id).orderBy("version", "desc").first();
        ...
        await db("documents").where("id", id).update({ updated_at: db.fn.now() });

        const updatedDoc = await db("documents").where("id", id).first();
