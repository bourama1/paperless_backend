...
export const reviseFile = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await db.run("INSERT INTO revisions (document_id, filename, version, annotations) VALUES (?, ?, ?, ?)", [
            id,
            newFilename,
            newVersion,
            annotations,
        ...
        const revisions = await db.all("SELECT * FROM revisions WHERE document_id = ? ORDER BY version DESC", id);
