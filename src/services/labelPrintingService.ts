...
const COUNTRY_CODES_PATH =
  ...
  path.join(__dirname, '../../config/country-codes.json');
...
function loadCountryCodes() {
  try {
    ...
    console.log(`[LABELS] Loaded ${Object.keys(countryCodeMap).length} country code mappings from ${COUNTRY_CODES_PATH}`);
  } catch (err: any) {
...
/**
 * SECONDARY (orange) sticker — stripped label.
 * Field positions from the second block of the captured aktualniCMD .prn.
 */
function generateSecondaryBlock(label: LabelRow): string {
  const pos3 = label.position.padStart(3, '0');
  const cc   = countryCode(label.deliveryCountry);
 ...
 *
 * tiskSekundarnihoStitku = "Ne" for ALL label types in the parametry sheet,
 * so only the primary block is ever sent to the printer.
 * The template parameter selects aktualniCMDinter vs aktualniCMD layout.
 */
function generateEzpl(label: LabelRow, template: string): Buffer {
...
export async function handleLabelPrinting(update: OrderUpdate): Promise<void> {
  ...
  // Filter rows for this cycle. Two independent rules must BOTH pass:
  //
  // Rule 1 — door index from packageType (col 4):
  //   "Sektion 2/2", "V - 1/2", "Tormatic 2/2" etc. encode which door this row belongs to.
  //   The numerator must equal cycleIndex. Rows without a slash print on every cycle.
  //
  // Rule 2 — cycleFilter from parametry sheet (posledniCisloPozice column):
  //   null    → no restriction, print on every cycle
  //   'first' → only print when cycleIndex === 1           (posledniCisloPozice = 1)
  //   'last'  → only print when cycleIndex === totalCycles (posledniCisloPozice = 0)

  const cycleRows = labelRows.filter(row => {
    // Rule 1: door index from packageType
    const match = row.packageType.match(/(\d+)\s*\/\s*(\d+)/);
    if (match && parseInt(match[1]!, 10) !== cycleIndex) return false;

    // Rule 2: cycleFilter from label type config
  ...
  console.log(`[LABELS] ${cycleRows.length} rows for cycle ${cycleIndex}/${totalCycles} (${labelRows.length} total in CSV)`);
...
const TMP_FILES_PATH = process.env.LABEL_TMP_FILES_PATH ||
...
/**
 * Prints a QR PNG file N times on Windows using PowerShell's Start-Process
 * with the "Print" verb — the standard way to print an image file from the
 * command line on Windows (uses the system default printer).
 *
...
async function printQrPng(pngPath: string, copies: number): Promise<void> {
  ...
  for (let i = 0; i < copies; i++) {
    const psCommand = `Start-Process -FilePath "${pngPath}" -Verb Print -WindowStyle Hidden`;
    console.log(`[QR] powershell -Command "${psCommand}" (copy ${i + 1}/${copies})`);
    await execFileAsync('powershell.exe', ['-NoProfile', '-Command', psCommand]);
    // Small delay between copies to avoid overlapping print spooler jobs
    if (i < copies - 1) {
