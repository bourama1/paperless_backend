...
describe("Workstation Controller", () => {
    ...
    describe("receiveOrderUpdate", () => {
        ...
        it("should return 400 if action is invalid", async () => {
            mockRequest = {
                body: { order: { _id: "ord1" }, action: "INVALID" },
            };
    ...
    describe("searchPbomHandler", () => {
        ...
        it("should handle errors", async () => {
            mockRequest = { query: { order_code: "12345" } };
            (searchPbom as jest.Mock).mockRejectedValue(
    ...
    describe("getWorkstationLog", () => {
        it("should return logs without filters", async () => {
            ...
            db.mockReturnValue({
                where: () => ({
                    orderBy: () => ({ limit: () => thenable(mockLogs) }),
                }),
                orderBy: mockOrderBy,
            });
            (getDb as jest.Mock).mockResolvedValue(db);

            await getWorkstationLog(
                mockRequest as Request,
                mockResponse as Response,
            );
            ...
            db.mockReturnValueOnce({
                ...
                .mockReturnValueOnce({
                    where: () => ({
                        andWhereNot: () => ({
                            count: () => ({
                                first: () => thenable(mockCountResult),
                            }),
                        }),
                    }),
                })
